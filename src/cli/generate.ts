...
export const generate = (
  ...
  for (const [entityName, manifest] of entities) {
    ...
    /*
     * A mapper per entity. Knack answers with `field_1` / `field_1_raw`; the
     * interface above describes `recipesName`. Without this the typed hook is a
     * bare cast — it compiles, and every property is undefined at runtime.
     *
     * These belong in the entity's hook file, not in components, so a component
     * never sees a Knack key. `check:keys` enforces that.
     */
    const fieldsAccess =
      quoteKey(entityName) === entityName ? `FIELDS.${entityName}` : `FIELDS[${quoteKey(entityName)}]`;

    mappers.push(
      `/** Raw Knack record -> {@link ${interfaceName}}. Prefers \`_raw\` per field. */\n` +
        `export const ${toCamelCase(`to ${singularize(entityName)}`)} = ` +
        `(record: KnackRawRecord): ${interfaceName} =>\n` +
        `  mapRecord<${interfaceName}>(record, ${fieldsAccess});`,
    );
  ...
  /*
   * Import only the types the emitted interfaces actually reference.
   *
   * The template ships with `noUnusedLocals`, so a fixed import list makes the
   * generated file fail the consuming app's own typecheck whenever the Knack
   * app happens not to use, say, an address field — which is most apps.
   * `KnackViewRef` is always emitted, on the VIEWS declaration.
   */
  const body = [...interfaces, ...viewBlocks, ...mappers].join('\n');
  ...
  const usedTypes = [
    ...OPTIONAL_TYPES.filter((name) => new RegExp(`\\b${name}\\b`).test(body)),
    'KnackViewRef',
    ...(mappers.length > 0 ? ['KnackRawRecord'] : []),
  ].sort();
...
${mappers.length > 0 ? "import { mapRecord } from '@fmc/knack-core';\n" : ''}
